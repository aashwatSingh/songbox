...
def _upload_track(client: TestClient, synthetic_wav: Path) -> str:
...
def test_purge_deletes_old_rejected_track(
    synthetic_wav: Path, authed_client: AuthedClient
) -> None:
    track_id = _upload_track(authed_client.client, synthetic_wav)
...
def test_purge_does_not_delete_recent_pending_track(
    synthetic_wav: Path, authed_client: AuthedClient
) -> None:
    track_id = _upload_track(authed_client.client, synthetic_wav)
...
def test_purge_never_deletes_passed_tracks_regardless_of_age(
    synthetic_wav: Path, authed_client: AuthedClient
) -> None:
    track_id = _upload_track(authed_client.client, synthetic_wav)
...
def test_purge_deletes_supplementary_attestation_declaration_too(
    ...
    try:
        ...
        resolve_response = client.post(
            f"/review-queue/{track_id}/resolve", json={"approve": False}
        )
