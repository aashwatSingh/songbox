...
- **`services/api/pyproject.toml` already has `ignore = ["B008"]` under `[tool.ruff.lint]`** (added during Task 4, ratified after review): flake8-bugbear's B008 flags function calls in argument defaults, which is exactly FastAPI's `Depends(...)` dependency-injection pattern — verified to genuinely fire on `Depends(get_identity)` and would fire on essentially every route handler from Task 9 onward. This is already handled; no future task needs to touch this file for this reason.
